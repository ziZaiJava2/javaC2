...
//		}
		int a = 0;
		for(int i = 0,j = 0;i < Math.max(source.length, newData.length); ){
			if(source[i] == newData[j]){
				result.inBoth[a++] = source[i];
				source[i] = 0;
				newData[j] = 0;
				i++;
				j++;
		...
		for(int i=0,j=a;i<MAX_SIZE;i++,j++){
			if(j<source.length){
				result.onlyInSource[i]=source[j];
			}else{
				result.onlyInSource[i]=0;
			}
		}
		
		for(int i=0,j=a;i<MAX_SIZE;i++,j++){
			if(j<newData.length){
				result.notInSource[i]=newData[j];
			}else{
				result.notInSource[i]=0;
			}
		}
		
		
		result.onlyInSource = source;
