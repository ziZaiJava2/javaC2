...
public class RosterReporter {
	...
	public String getReport() {
		...
		String studentName = " ";
		for (int i = 0; i < reporter.allStudent.size(); i++) {
			studentName = studentName + reporter.allStudent.size() + "\n";
		}

		// 这个新的类有一个方法getReport()，返回一个字符串。字符串是对于构造函数传入的课程的描述:
		String Reporter = ("课程名称：" + reporter.getDepartment() + " "+ "编号" + reporter.getNumber() + " "+"参加学习学生总数"
				+ reporter.getNumberOfStudents() + " "+"开课日期" + format1.format(reporter.getStartDate()) + "结束日期"
				+ format1.format(reporter.getEndDate()) + studentName);
		return Reporter;
