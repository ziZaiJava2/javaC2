...
public class Test4 {
	...
	public static Result merge(int[] source, int[] newData) {
		...
		// 这里写计算过程：
		result.notInSource = newData;
		int time = 0;
		int x = 0;
		for (int i = 0; i < source.length; i++) {
			int countForSource = time;
			...
			if (countForSource == newData.length) {
				result.onlyInSource[x] = source[x];
				x += 1;
			}

		}
		int[] arr = new int[result.notInSource.length];
		for (int i = 0; i < result.notInSource.length; i++) {
			if (result.notInSource[i] != 0) {
				arr[i] = result.notInSource[i];
			}
		}
		result.notInSource = arr;
