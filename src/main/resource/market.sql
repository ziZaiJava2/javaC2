...
update orders set state = '已付款' where id = 5;
...
#然后付款，写出SQL。
update orders set state ='已付款' where id = 6;
...
insert into orders(user_id,create_date,address,total,state)
  values(4,'2016-10-15','上海电机学院','1990','未付款');
...
#各种商品及他们销售量的列表
select sum(price * count) from orders o,orders_production op 
where o.id=op.orders_id and state = '已付款';
